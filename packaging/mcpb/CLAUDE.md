...
## The six wrapper vectors and the ordering rules

Around the `.mcpb` bundles this directory owns six thin, version-locked wrappers, all on one contract:
...
The Gleam, Kotlin and Swift SDKs that once existed as `stackql/stackql-mcp-{gleam,kotlin,swift}` were retired in 2026-08 (fringe audiences, disproportionate registry/toolchain cost); their archived repos hold the code if ever wanted. Only the three SDKs above are supported.
...
- Pins are data: `platforms.json` `{version, baseUrl, platforms{<key>:{bundle, sha256}}}`, written by `scripts/render-platforms.sh` (`make manifests VERSION=X.Y.Z`, or `make <vector>-manifest`) from the published `.mcpb.sha256` release assets. It is the ONLY pin source: there is no hand-written pin table anywhere (Rust `build.rs` renders consts from it, Go `go:embed`s it, .NET embeds it as a resource). The rendered files are gitignored; every vector's build fails loudly with a pointer at `make <vector>-manifest` when it is missing.
...
3. Then publish wrappers in any order (npm, pypi manual; oci + cargo/go/dotnet from the `mcp-packaging` dispatch, which validates every SDK slice with a real proxy download before any publish job runs).
...
Go modules cannot be consumed from a monorepo subdirectory cleanly, so `go/` is the source of truth here and `scripts/publish-mirror.sh` pushes the rendered subtree to the pre-existing mirror repo `stackql/stackql-mcp-go` and tags `v<version>` (idempotent, immutable). Demo apps (auditron, stackql-agent, sandboxctl, driftwatch) do NOT live here; they belong in stackql-labs and depend on the published packages.

Cache layout note: the npm/PyPI wrappers and the Go SDK extract only the entry point (`<key>/stackql`); the Rust and .NET SDKs extract the whole bundle (`<key>/server/stackql`, plus manifest/README) into the same directory. Same directory, different file layout - they coexist but do not reuse each other's binary.
