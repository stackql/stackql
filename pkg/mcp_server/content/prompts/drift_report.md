...
5. **Query live state.** For each mapped resource type, list the live resources in the declared regions/projects/subscriptions (widen to all accessible regions when the intent covers a type globally; the `stackql://docs/scope_discovery` resource carries tested enumeration queries where the client exposes resources). Keep result sets bounded with `row_limit` where supported.
