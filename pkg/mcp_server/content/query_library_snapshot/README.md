...
- **Mastered in**: the `stackql.io` repository, under `query-library/`
  (published to `https://stackql.io/docs/query-library`). Author, fix and
  extend entries there, never here.
...
- **Refreshing**: copy the generated artifacts (`manifest.json`, `index.json`,
  `queries/**/*.json`) from the stackql.io repo's `static/docs/query-library/`
  output. Keep the set small - this ships in every binary; it is a survival
