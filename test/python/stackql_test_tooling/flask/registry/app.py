from flask import Flask, request, Request, render_template, make_response, jsonify, send_file
import os
import logging
import re
import json
...
def generic_handler(request: Request):
    ...
    if not template_name:
        file_name = route_cfg.get("httpResponse", {}).get("file", "")
        if file_name:
            logger.info(f"Returning file: {file_name}")
            content_type = route_cfg.get("httpResponse", {}).get("headers", {}).get("Content-Type", "application/octet-stream")
            return send_file(os.path.join(app.template_folder, file_name), as_attachment=True, mimetype=content_type)
        logger.error(f"Missing response definition for route: {request}")
